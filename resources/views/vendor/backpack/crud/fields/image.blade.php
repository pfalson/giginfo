<div class="form-group col-md-12 image" data-preview="#{{ $field['name'] }}"
     ...
     data-max_size="{{ isset($field['max_size']) ? $field['max_size'] : false }}"
        @include('crud::inc.field_wrapper_attributes')>
    <div>
        <label>{!! $field['label'] !!}</label>
    </div>
    <!-- Wrap the image or canvas element with a block element (container) -->
    <?php $value = isset($field['value']) ? $field['value'] : (isset($field['default']) ? $field['default'] : ''); ?>
    <div class="row" {!! (empty($value) ? ' style="display:none"' : '') !!}>
        <div class="col-sm-6" style="margin-bottom: 20px;">
            <?php $value = isset($field['value']) ? $field['value'] : (isset($field['default']) ? $field['default'] : ''); ?>
            <img id="mainImage"
                 src="{{ url(old($field['name']) ? old($field['name']) : $value) }}" {{ (empty($value) ? ' style="display:none"' : '') }}>
        </div>
            ...
            $('.form-group.image').each(function (index) {
                ...
                $uploadImage.change(function () {
                    ...
                    if (/^image\/\w+$/.test(file.type)) {
                        var filesizeMax = file.size;
                        if (max_size.length > 0) {
                            filesizeMax = +max_size;
                        }
                        if (file.size > filesizeMax) {
                            alert('File size ' + file.size + ' exceeds max (' + filesizeMax + ')');
                        } else {
