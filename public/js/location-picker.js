$.fn.locationPicker = function(options){
    ...
    OpenLayers.Control.Click = OpenLayers.Class(OpenLayers.Control, {
        ...
        trigger: function(e) {
            ...
            data = {
                location: {lat:latLong.lat, long: latLong.lon}
            }

            // TODO init map with reverse geocode
            geoCoder.geocode({location: new google.maps.LatLng(latLong.lat,latLong.lon)}, function(result, status){
                if( status == 'OK'){
    ...
    if( !mapId ) {mapEl.attr('id', generateRandId() )}
    ...
    this.setAddress = function(address){
        setAddressInternal({address: address})
    }

    this.setLocation = function(lat, long){
        setAddressInternal({location: new google.maps.LatLng(lat,long)})
    }
